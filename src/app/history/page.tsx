...
const CAT_COLOR: Record<string, React.CSSProperties> = {
  '餐飲':{background:'#FDF3E3',color:'#9A6020'},'交通':{background:'#E8F0FB',color:'#00695C'},
    ...
    return (
      <div className="pb-24 px-4 pt-6">
        <div className="flex items-center gap-3 mb-5">
          <button onClick={() => setDetail(null)} className="w-9 h-9 bg-white rounded-full flex items-center justify-center text-base" style={{border:'0.5px solid #C8E0DC',color:'#37474F'}}>←</button>
          <h1 className="text-xl font-bold" style={{color:'#37474F'}}>支出細項</h1>
        </div>
        <div style={{borderRadius:'16px',overflow:'hidden',border:'0.5px solid #C8E0DC',marginBottom:'12px'}}>
          <div style={{background:'#26A69A',padding:'16px'}}>
          ...
          <div style={{background:'white'}}>
            {([
              ...
              <div key={k} className="flex justify-between px-4 py-2.5" style={{borderBottom:'0.5px solid #E0F7F5',fontSize:'13px'}}>
                <span style={{color:'#80B5B0'}}>{k}</span>
                <span className="font-medium" style={{color:'#37474F'}}>{v}</span>
              </div>
            ))}
          </div>
          <div className="flex gap-2 p-3" style={{background:'#F8FBFB',borderTop:'0.5px solid #C8E0DC'}}>
            <Link href={`/add?edit=${detail.id}`} className="flex-1 py-2.5 rounded-xl text-center text-sm font-medium" style={{background:'white',border:'0.5px solid #C8E0DC',color:'#37474F'}}>✏️ 編輯</Link>
  ...
  return (
    <div className="pb-24 px-4 pt-6">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-bold" style={{color:'#37474F'}}>歷史記錄</h1>
        <span className="text-sm" style={{color:'#80B5B0'}}>{filtered.length} 筆</span>
      </div>
      <div className="flex gap-2 overflow-x-auto pb-2 mb-2">
        {CATS.map(c => (
          <button key={c} onClick={() => setCat(c)} className="px-3 py-1.5 rounded-full text-sm whitespace-nowrap flex-shrink-0 font-medium"
            style={cat===c ? {background:'#26A69A',color:'white'} : {background:'white',color:'#555',border:'0.5px solid #C8E0DC'}}>
            {c}
          </button>
        ))}
      </div>
      <div className="flex gap-2 overflow-x-auto pb-3 mb-3">
        {REGIONS.map(r => (
          <button key={r} onClick={() => setRegion(r)} className="px-3 py-1.5 rounded-full text-sm whitespace-nowrap flex-shrink-0 font-medium"
            style={region===r ? {background:'#37474F',color:'white'} : {background:'white',color:'#555',border:'0.5px solid #C8E0DC'}}>
            {r}
          </button>
        ))}
      </div>
      {loading ? (
        ...
        <div className="card text-center py-8" style={{color:'#80B5B0'}}>沒有符合的記錄</div>
      ) : (
            ...
            <div key={r.id||i} className="card">
              <div className="flex justify-between items-start mb-2">
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-sm" style={{color:'#37474F'}}>{r.items || r.storeName}</p>
                  <div className="flex items-center gap-1.5 mt-1">
                    ...
                    {r.user && <span className="text-xs" style={{color:'#80B5B0'}}>· {r.user}</span>}
                  </div>
                </div>
                <div className="text-right ml-3">
                  <p className="font-bold text-sm" style={{color:'#37474F'}}>¥{r.amountJPY.toLocaleString()}</p>
                  <p className="text-xs mt-0.5" style={{color:'#00A86B'}}>NT${Math.round(r.amountJPY * exchangeRate).toLocaleString()}</p>
                </div>
              </div>
              <div className="flex gap-2 pt-2" style={{borderTop:'0.5px solid #E0F7F5'}}>
                <Link href={`/add?edit=${r.id}`} className="flex-1 py-1.5 rounded-xl text-center text-xs font-medium" style={{background:'#F8FBFB',color:'#555'}}>編輯</Link>
                <button onClick={() => setDetail(r)} className="flex-1 py-1.5 rounded-xl text-xs font-medium" style={{background:'#E0F7F5',color:'#26A69A'}}>查看細項</button>
