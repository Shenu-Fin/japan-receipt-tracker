...
const CAT_COLOR: Record<string, React.CSSProperties> = {
  '餐飲':{background:'#FDF3E3',color:'#9A6020'},'交通':{background:'#E8F0FB',color:'#1A7A77'},
    ...
    return (
      <div className="pb-24 px-4 pt-6">
        <div className="flex items-center gap-3 mb-5">
          <button onClick={() => setDetail(null)} className="w-9 h-9 bg-white rounded-full flex items-center justify-center text-base" style={{border:'0.5px solid #D4E6E4',color:'#455A64'}}>←</button>
          <h1 className="text-xl font-bold" style={{color:'#455A64'}}>支出細項</h1>
        </div>
        <div style={{borderRadius:'16px',overflow:'hidden',border:'0.5px solid #D4E6E4',marginBottom:'12px'}}>
          <div style={{background:'#20B2AA',padding:'16px'}}>
          ...
          <div style={{background:'white'}}>
            {([
              ...
              <div key={k} className="flex justify-between px-4 py-2.5" style={{borderBottom:'0.5px solid #E0F2F1',fontSize:'13px'}}>
                <span style={{color:'#8BBAB8'}}>{k}</span>
                <span className="font-medium" style={{color:'#455A64'}}>{v}</span>
              </div>
            ))}
          </div>
          <div className="flex gap-2 p-3" style={{background:'#F0F4F4',borderTop:'0.5px solid #D4E6E4'}}>
            <Link href={`/add?edit=${detail.id}`} className="flex-1 py-2.5 rounded-xl text-center text-sm font-medium" style={{background:'white',border:'0.5px solid #D4E6E4',color:'#455A64'}}>✏️ 編輯</Link>
  ...
  return (
    <div className="pb-24 px-4 pt-6">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-bold" style={{color:'#455A64'}}>歷史記錄</h1>
        <span className="text-sm" style={{color:'#8BBAB8'}}>{filtered.length} 筆</span>
      </div>
      <div className="flex gap-2 overflow-x-auto pb-2 mb-2">
        {CATS.map(c => (
          <button key={c} onClick={() => setCat(c)} className="px-3 py-1.5 rounded-full text-sm whitespace-nowrap flex-shrink-0 font-medium"
            style={cat===c ? {background:'#20B2AA',color:'white'} : {background:'white',color:'#555',border:'0.5px solid #D4E6E4'}}>
            {c}
          </button>
        ))}
      </div>
      <div className="flex gap-2 overflow-x-auto pb-3 mb-3">
        {REGIONS.map(r => (
          <button key={r} onClick={() => setRegion(r)} className="px-3 py-1.5 rounded-full text-sm whitespace-nowrap flex-shrink-0 font-medium"
            style={region===r ? {background:'#455A64',color:'white'} : {background:'white',color:'#555',border:'0.5px solid #D4E6E4'}}>
            {r}
          </button>
        ))}
      </div>
      {loading ? (
        ...
        <div className="card text-center py-8" style={{color:'#8BBAB8'}}>沒有符合的記錄</div>
      ) : (
            ...
            <div key={r.id||i} className="card">
              <div className="flex justify-between items-start mb-2">
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-sm" style={{color:'#455A64'}}>{r.items || r.storeName}</p>
                  <div className="flex items-center gap-1.5 mt-1">
                    ...
                    {r.user && <span className="text-xs" style={{color:'#8BBAB8'}}>· {r.user}</span>}
                  </div>
                </div>
              ...
              <div className="flex gap-2 pt-2" style={{borderTop:'0.5px solid #E0F2F1'}}>
                <Link href={`/add?edit=${r.id}`} className="flex-1 py-1.5 rounded-xl text-center text-xs font-medium" style={{background:'#F0F4F4',color:'#555'}}>編輯</Link>
                <button onClick={() => setDetail(r)} className="flex-1 py-1.5 rounded-xl text-xs font-medium" style={{background:'#E0F2F1',color:'#20B2AA'}}>查看細項</button>
