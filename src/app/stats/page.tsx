...
const COLORS = ['#0A7A70','#1A1A1B','#B07A00','#8A208A','#3A7A20','#AA2020','#3A20AA','#6B5A00']
const NA = '#D0F567', NI = 'rgba(255,255,255,0.45)'

function PieChart({data}:{data:{label:string;value:number;color:string}[]}) {
  const total = data.reduce((s,d)=>s+d.value,0)
  if(!total) return <div className="w-24 h-24 rounded-full" style={{background:'#ECECE8'}}/>
  let offset=25
...
function Section({title,data,labelKey}:{title:string;data:{label:string;value:number}[];labelKey:string}) {
  ...
  return (
    <div className="card mb-3">
      <p className="text-xs font-semibold mb-3" style={{color:'#ABABAB',letterSpacing:'0.06em',textTransform:'uppercase'}}>{title}</p>
          ...
          {colored.map((d,i)=>(
            <div key={i} className="flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs" style={{color:'#1A1A1B'}}>{d.label}</span>
              </div>
      ...
      <div style={{borderTop:'0.5px solid #E8E8E4',paddingTop:'10px'}}>
        <div className="grid grid-cols-4 gap-1 mb-2" style={{fontSize:'11px',color:'#ABABAB'}}>
          <span>#</span><span>{labelKey}</span><span className="text-right">金額</span><span className="text-right">比例</span>
        </div>
        {colored.map((d,i)=>(
          <div key={i} className="grid grid-cols-4 gap-1 py-2" style={{borderBottom:i<colored.length-1?'0.5px solid #F0F0EC':'none',fontSize:'13px'}}>
            <span style={{color:'#ABABAB',fontSize:'11px'}}>{i+1}</span>
            <span className="font-medium" style={{color:'#1A1A1B'}}>{d.label}</span>
            <span className="text-right font-semibold" style={{color:d.color}}>¥{d.value.toLocaleString()}</span>
            <span className="text-right font-semibold" style={{color:i===0?d.color:'#ABABAB'}}>{d.pct}%</span>
          </div>
        ))}
      </div>
    </div>
...
export default function StatsPage() {
  ...
  useEffect(()=>{
    ...
    fetch('/api/exchange-rate').then(r=>r.json()).then(d=>setRate(d.rate||0.21))
  },[])
  ...
  if(loading) return <div className="p-8 text-center pt-16" style={{color:'#ABABAB'}}>載入中...</div>

  return (
    <div className="pb-24 px-4 pt-6">
      <h1 className="text-xl font-bold mb-4" style={{color:'#1A1A1B'}}>統計分析</h1>

      {/* Hero 總花費 */}
      <div style={{background:'#1A1A1B',borderRadius:'20px',padding:'20px',marginBottom:'16px'}}>
        <p className="text-xs mb-1" style={{color:'rgba(255,255,255,0.55)'}}>旅程總花費</p>
        ...
        <div className="flex justify-between items-center">
          <span className="text-xs" style={{color:'rgba(255,255,255,0.55)'}}>換算台幣</span>
          <span className="font-bold" style={{color:'#B2F0EB'}}>NT${totalTWD.toLocaleString()}</span>
        </div>
        <p className="text-xs mt-1" style={{color:'rgba(255,255,255,0.3)'}}>即時匯率 ¥1 ≈ NT${rate.toFixed(4)}</p>
      </div>

      {Object.keys(byCat).length>0  && <Section title="支出類別比" data={Object.entries(byCat).sort((a,b)=>b[1]-a[1]).map(([l,v])=>({label:l,value:v}))} labelKey="類別"/>}
      {Object.keys(byPay).length>0  && <Section title="支出帳戶比" data={Object.entries(byPay).sort((a,b)=>b[1]-a[1]).map(([l,v])=>({label:l,value:v}))} labelKey="帳戶"/>}
      {Object.keys(byUser).length>0 && <Section title="支出成員比" data={Object.entries(byUser).sort((a,b)=>b[1]-a[1]).map(([l,v])=>({label:l,value:v}))} labelKey="成員"/>}
      {receipts.length===0 && <div className="card text-center py-8" style={{color:'#ABABAB'}}>還沒有記錄</div>}
