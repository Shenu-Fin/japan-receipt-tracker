...
const COLORS = ['#4785FF','#00A86B','#FF6B6B','#FFB347','#9B59B6','#1ABC9C','#E67E22','#3498DB']
const NA = '#00A86B'
const NI = '#8FA8CC'

function PieChart({ data }: { data: { label: string; value: number; color: string }[] }) {
  const total = data.reduce((s, d) => s + d.value, 0)
  if (total === 0) return <div className="w-24 h-24 rounded-full" style={{background:'#E4ECFF'}} />
  let offset = 25
...
function Section({ title, data, labelKey }: { title: string; data: { label: string; value: number }[]; labelKey: string }) {
  ...
  return (
    <div className="card mb-3">
      <p className="text-xs font-semibold mb-3" style={{color:'#8FA8CC',letterSpacing:'0.06em',textTransform:'uppercase'}}>{title}</p>
          ...
          {colored.map((d, i) => (
            <div key={i} className="flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs" style={{color:'#333333'}}>{d.label}</span>
              </div>
      ...
      <div style={{borderTop:'0.5px solid #E4ECFF',paddingTop:'10px'}}>
        <div className="grid grid-cols-4 gap-1 mb-2" style={{fontSize:'11px',color:'#8FA8CC'}}>
          <span>#</span><span>{labelKey}</span><span className="text-right">金額</span><span className="text-right">比例</span>
        </div>
        {colored.map((d, i) => (
          <div key={i} className="grid grid-cols-4 gap-1 py-2" style={{borderBottom: i < colored.length-1 ? '0.5px solid #EEF3FF' : 'none', fontSize:'13px'}}>
            <span style={{color:'#8FA8CC',fontSize:'11px'}}>{i+1}</span>
            <span className="font-medium" style={{color:'#333333'}}>{d.label}</span>
            <span className="text-right font-semibold" style={{color: d.color}}>¥{d.value.toLocaleString()}</span>
            <span className="text-right font-semibold" style={{color: i===0 ? d.color : '#8FA8CC'}}>{d.pct}%</span>
          </div>
        ))}
      </div>
    </div>
...
export default function StatsPage() {
  ...
  if (loading) return <div className="p-8 text-center pt-16" style={{color:'#8FA8CC'}}>載入中...</div>

  return (
    <div className="pb-24 px-4 pt-6">
      <h1 className="text-xl font-bold mb-4" style={{color:'#333333'}}>統計分析</h1>

      <div style={{background:'#4785FF',borderRadius:'20px',padding:'18px',marginBottom:'16px'}}>
      ...
      {receipts.length === 0 && <div className="card text-center py-8" style={{color:'#8FA8CC'}}>還沒有記錄</div>}
